...
unsigned long samplingRate = 11025;
...
void Fill_Buffer(void *pvParameters) {
  ...
  for (;;) {
    if (Current_Buffer == 1) {
      flrdsz = file.read((byte *)Buffer[0], totalSampleWords * 2);
    } else {
      flrdsz = file.read((byte *)Buffer[1], totalSampleWords * 2);
    }
